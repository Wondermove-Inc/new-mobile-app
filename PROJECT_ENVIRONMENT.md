...
- Root Claude runtime artifacts are not part of the active Codex runtime. They are
  ignored transient files and may be regenerated by local Claude/headless tooling
  without failing repo runtime validation:
  - `CLAUDE.md`
  - `.claude/`
  - `.claude-state/`
  - `scripts/validate-runtime-artifacts.mjs` requires these root paths to remain
    covered by `.gitignore`; third-party files with the same names under ignored
    dependency directories are outside this policy.
...
- Runtime scripts:
  - `scripts/validate-runtime-artifacts.mjs`
    - The root `validate` package script removes transient `.claude/` and `.claude-state/` before running this validator, while the validator itself permits regenerated root Claude transient artifacts only when `.gitignore` covers them.
