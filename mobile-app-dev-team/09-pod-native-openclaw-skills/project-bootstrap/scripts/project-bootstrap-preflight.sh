...
function buildUserUnderstandableResult() {
  ...
  if (hasPodRoleBlocked) {
    lines.push('The pod role bootstrap report exists, but the pod is not ready yet.');
    lines.push('`project-bootstrap` is surfacing the nested pod role readiness result; it is not claiming the pod is ready.');
  } else {
    lines.push('Project bootstrap found setup work that must be resolved before the pod can continue safely.');
  }
  ...
  if (hasGitIdentityMissing) {
    lines.push('- Git identity is still missing because no approved non-secret Git identity pair is available to the agent.');
  }
  if (hasGithubAuthUnavailable) {
    lines.push('- GitHub auth is still unavailable because no authenticated `gh` state or approved mounted/managed GitHub auth source is available to the agent.');
  }
