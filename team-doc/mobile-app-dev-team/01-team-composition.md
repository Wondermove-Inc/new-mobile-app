...
| Role | Type | Mission |
| --- | --- | --- |
| Product/Planning | LLM | 요구사항을 executable work로 정리하고 scope, non-goal, human gate, readiness를 관리한다. |
| Design | LLM | approved requirement를 implementation-ready mobile UX handoff로 만든다. |
| Mobile Architect | LLM | Expo/RN app architecture가 일관되고 release 가능한 상태를 유지한다. |
| Mobile App Dev | LLM | 승인된 task, design handoff, API contract에 따라 Expo React Native 기능을 구현한다. |
| Backend/API Integrator | LLM | mobile-facing API contract, schema, mock, fixture, auth/session, error mapping을 안전하게 관리한다. |
| QA/Release | LLM | RN Web, Maestro, mobile-mcp, EAS, Railway evidence로 release 상태를 측정 가능하게 만든다. |
| Gatekeeper | non-LLM | deterministic required check로 pass/fail을 판정한다. |
